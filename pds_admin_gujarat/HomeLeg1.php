...
<ul class="breadcrumb">
	<li><a href="#">Home</a></li>
	<li class="active">Gujarat PDS Route Optimisation</li>
</ul>
<!-- END BREADCRUMB -->
<div>
		...
		<div class="col-md-12">
			...
			<div class="panel panel-default">
				<div class="panel-heading" style="text-align: center;">
					<h1 style="font-weight: bold; color: #335566;">Gujarat PDS Route Optimisation</h1>
...
<script>
						...
						.then(data => {
							...
							fetch(pythonUrl + 'getfcidataleg1', {
								...
								.then(data => {
									...
									if (!window.isJobRunning) { document.getElementById("processingPopup").style.display = "none"; }
    ...
    function generateoptimizedplan() {
        ...
        .then(data => {
            if (data.status === 1 && data.job_id) {
                ...
                renderOptimizedTable(data);
            } else {
                throw new Error(data.message || "Unknown error starting job");
            }
        })
        .catch(error => {
    ...
    function pollJobStatus(job_id) {
        ...
        pollInterval = setInterval(() => {
            fetch(pythonUrl + 'job_status/' + job_id)
                .then(response => response.json())
                .then(data => {
                    if (data.status === 1 && data.job) {
                        if (data.job.status === "completed") {
                            clearInterval(pollInterval);
                            fetch(pythonUrl + 'job_result/' + job_id)
                                .then(res => res.json())
                                .then(resultData => {
                                    renderOptimizedTable(resultData);
                                });
                        } else if (data.job.status === "failed") {
                            clearInterval(pollInterval);
                            alert("Job Failed: " + data.job.message);
                            resetUI();
...
function downloadExcelFile() {
    ...
    a.href = url;
    a.download = 'Template_SCO_IntraState_Gujarat.xlsx';
    a.click();
...
function downloadFile(fileType) {
    ...
    if (fileType === 'xlsx') {
        filePath = 'template/Template_SCO_IntraState_Gujarat.xlsx';
        fileName = 'Template_SCO_IntraState_Gujarat.xlsx';
    } else if (fileType === 'pdf') {
        filePath = 'template/Template_SCO_IntraState_Gujarat.pdf';
        fileName = 'Template_SCO_IntraState_Gujarat.pdf';
    }
	...
	if (checkbox.checked) {
		...
		fetch(pythonUrl + 'getGraphDataleg1', {
			...
			.then(data => {
				...
				console.log("Total demand:", totalDemandFRice);

				document.getElementById("totalFciDemand").innerHTML = "<span style='color: white; font-size: 14px;'>" + "Total Demand: " + totalDemand + " (Qtl)</span>";
				document.getElementById("totalFciSupply").innerHTML = "<span style='color: white; font-size: 14px;'>" + "Total Supply Wheat: " + totalCapacity + " (Qtl)</span>";
				...
				if (totalCapacity > -1 && totalDemand > -1 && totalCapacity1 > -1 && totalDemandRice > -1 && totalCapacity2 > -1 && totalDemandFRice > -1 ) {
...
function showCheckboxes() {
  ...
  if (!expanded) {
    ...
    function checkActiveJob() {
        ...
        fetch(pythonUrl + 'active_job?client_id=' + encodeURIComponent(username) + '&endpoint=' + encodeURIComponent('/processFileleg1'))
            .then(response => response.json())
            .then(data => {
                if (data.status == 1 && data.job) {
                    window.isJobRunning = true;
